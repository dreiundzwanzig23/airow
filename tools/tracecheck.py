...
Generates compact docs/process/TRACEABILITY.md summary output. Use --json for
the complete trace graph.
"""
...
def write_traceability(data: Dict[str, object], warnings: List[str]) -> None:
    ...
    for test_id, fields in data["UT"].items():
        if fields.get("Aux", False):
            continue
        for design_id in fields.get("Verifies", []):
            ut_by_d.setdefault(design_id, []).append(test_id)

    it_by_a: Dict[str, List[str]] = {}
    for test_id, fields in data["IT"].items():
        if fields.get("Aux", False):
            continue
        for arch_id in fields.get("Verifies", []):
            it_by_a.setdefault(arch_id, []).append(test_id)

    qt_by_r: Dict[str, List[str]] = {}
    for test_id, fields in data["QT"].items():
        if fields.get("Aux", False):
            continue
        for req_id in fields.get("Verifies", []):
            qt_by_r.setdefault(req_id, []).append(test_id)

    def count_by_field(items: Dict[str, Dict[str, object]], field: str) -> Dict[str, int]:
        counts: Dict[str, int] = {}
        for fields in items.values():
            value = str(fields.get(field, "UNSET"))
            counts[value] = counts.get(value, 0) + 1
        return counts

    def format_counts(counts: Dict[str, int], order: List[str]) -> str:
        ordered_parts = [f"{key}: {counts[key]}" for key in order if key in counts]
        ordered_parts.extend(
            f"{key}: {counts[key]}" for key in sorted(counts.keys()) if key not in order
        )
        return ", ".join(ordered_parts) if ordered_parts else "none"

    requirement_status = count_by_field(requirements, "Status")
    architecture_status = count_by_field(architecture, "Status")
    architecture_profiles: Dict[str, int] = {}
    for fields in architecture.values():
        profile = architecture_evidence_profile(fields)
        architecture_profiles[profile] = architecture_profiles.get(profile, 0) + 1

    non_aux_test_counts = {
        test_kind: sum(1 for fields in data[test_kind].values() if not fields.get("Aux", False))
        for test_kind in ("UT", "IT", "QT")
    }
    aux_count = sum(
        1
        for test_kind in ("UT", "IT", "QT")
        for fields in data[test_kind].values()
        if fields.get("Aux", False)
    )
    needs_review = sum(
        1
        for fields in requirements.values()
        if str(fields.get("Needs-Review", "")).strip().lower() == "yes"
    )

    requirements_with_arch = sum(1 for req_id in requirements if a_by_r.get(req_id))
    done_requirements = [
        req_id for req_id, fields in requirements.items() if fields.get("Status") == "DONE"
    ]
    done_requirements_with_qt = sum(1 for req_id in done_requirements if qt_by_r.get(req_id))
    code_architecture = [
        arch_id
        for arch_id, fields in architecture.items()
        if architecture_evidence_profile(fields) == "CODE"
    ]
    code_architecture_with_design = sum(
        1 for arch_id in code_architecture if d_by_a.get(arch_id)
    )
    code_architecture_with_it = sum(
        1 for arch_id in code_architecture if it_by_a.get(arch_id)
    )
    ...
    designs_with_qualifying_ut = sum(
        1 for design_id in design if design_has_qualifying_ut(design_id)
    )
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- Requirements: {len(requirements)} ({format_counts(requirement_status, ['DONE', 'IN_PROGRESS', 'OPEN'])})",
        f"- Requirements needing review: {needs_review}",
        f"- Architecture items: {len(architecture)} ({format_counts(architecture_status, ['DONE', 'IN_PROGRESS', 'OPEN'])})",
        f"- Architecture evidence profiles: {format_counts(architecture_profiles, ['CODE', 'ASSET'])}",
        f"- Design blocks: {len(design)}",
        f"- Evidence tests: UT {non_aux_test_counts['UT']}, IT {non_aux_test_counts['IT']}, QT {non_aux_test_counts['QT']}",
        f"- Auxiliary tests excluded from evidence gates: {aux_count}",
        "",
        "## Evidence Coverage",
        "",
        f"- Requirements with architecture allocation: {requirements_with_arch}/{len(requirements)}",
        f"- DONE requirements with requirement-level QT evidence: {done_requirements_with_qt}/{len(done_requirements)}",
        f"- CODE architecture items with design coverage: {code_architecture_with_design}/{len(code_architecture)}",
        f"- CODE architecture items with IT evidence: {code_architecture_with_it}/{len(code_architecture)}",
        f"- Design blocks with qualifying UT evidence: {designs_with_qualifying_ut}/{len(design)}",
        "",
        "## Design Refinement Links",
        "",
    ]
    lines.extend(["| Design | Refines Design |", "|--------|----------------|"])
