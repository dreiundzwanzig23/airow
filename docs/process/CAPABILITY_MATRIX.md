...
| Visualization | implemented-but-unvalidated | Runs can emit `airow.visualization.v1` JSON with frame, channel, transform, vector, provider, backend, and trust metadata for downstream tools. | Interactive 3D playback, configurable reference-frame selection, VTK/ParaView export, and validated visualization workflows remain future work. |
