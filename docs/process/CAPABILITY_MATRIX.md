...
| Visualization | implemented-but-unvalidated | Runs can emit `airow.visualization.v1` JSON with frame, channel, transform, vector, provider, backend, and trust metadata, and the offline report exposes playback, projection, frame-label, vector-toggle, and linked-plot controls for emitted channels. | Interactive 3D playback, transformed local-frame channels, VTK/ParaView export, and validated visualization workflows remain future work. |
