...
| Visualization | implemented-but-unvalidated | Runs can emit `airow.visualization.v1` JSON with frame, channel, transform, vector, provider, backend, and trust metadata; the offline report exposes playback, projection, frame-label, vector-toggle, and linked-plot controls for emitted channels; and validated artifacts can be exported to a reduced ParaView/VTK bundle. | Interactive 3D playback, interface/disturbance vector coverage, production geometry surfaces, ParaView loading guides, and validated visualization workflows remain future work. |
