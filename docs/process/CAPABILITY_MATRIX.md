...
| Visualization | implemented-but-unvalidated | Runs can emit `airow.visualization.v1` JSON with frame, channel, transform, vector, provider, backend, and trust metadata; the offline report exposes physics-capability, playback, projection, frame-label, vector-toggle, linked-plot, and export-guide controls for emitted channels; and validated artifacts can be exported to a reduced ParaView/VTK bundle with a loading guide. | Interactive 3D playback, interface/disturbance vector coverage, production geometry surfaces, and validated visualization workflows remain future work. |
