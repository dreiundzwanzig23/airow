 ...
 * @test UT-012
 * @verifies [D-014]
 * @notes Given CLI-style arguments, when the headless CLI wrapper runs, then
 * usage handling and stable success or configuration-failure exit mapping are
 * deterministic without requiring the real process entry point.
 */
TEST(SimulationRun, HeadlessCliWrapperMapsUsageAndRunStatuses) {
